    ...
    if (warp == 8) {
        { // mma_warp_main
            ...
            for (unsigned int work_idx_4 = bid / 2; work_idx_4 < total_work_items; work_idx_4 += num_bids / 2) {
                ...
                {
                    {
                        mbarrier_wait_cluster_hint(s_seeded_addr, _phase_s_seeded_0, 10000000);
                        _phase_s_seeded_0 ^= 1;
                    }
                    asm volatile("tcgen05.fence::after_thread_sync;");
                }
                    ...
                    int score_col = ((phase_2 != 0) ? 128 : 0);
                    {
                        ...
                        for (int n = 0; n < 4; n++) {
                            ...
                            asm volatile("tcgen05.fence::after_thread_sync;");
                            {
                                ...
                                if (cta_rank == 0) {
                                    mbarrier_wait_cluster_hint(kv_pair_ready_addr + (mma_kv_stage) * 8, _phase_kv_pair_ready, 10000000);
                                    asm volatile("tcgen05.fence::after_thread_sync;");
                            ...
                            for (int vs_2 = 0; vs_2 < 2; vs_2++) {
                                ...
                                int output_col = 256 + vs_2 * 128;
                                {
                                    ...
                                    if (cta_rank == 0) {
                                        mbarrier_wait_cluster_hint(kv_pair_ready_addr + (mma_kv_stage) * 8, _phase_kv_pair_ready, 10000000);
                                        asm volatile("tcgen05.fence::after_thread_sync;");
                                        int _mma_b_lo_9 = ((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (mma_kv_stage) * 1024;
                                        asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ...
                    for (int vs_3 = 0; vs_3 < 2; vs_3++) {
                        ...
                        int output_col_d = 256 + vs_3 * 128;
                        {
                            ...
                            if (cta_rank == 0) {
                                mbarrier_wait_cluster_hint(kv_pair_ready_addr + (mma_kv_stage) * 8, _phase_kv_pair_ready, 10000000);
                                asm volatile("tcgen05.fence::after_thread_sync;");
                                int _mma_b_lo_13 = ((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (mma_kv_stage) * 1024;
                                asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id, m0, m1, m2, m3, m4, m5, m6, m7;\n\t"
