...
#include "cake_dsv4_bf16_h32_topk128x_early_v47.cu"
#undef CakeTensorMap
#undef uint8_t
#undef uint16_t
#undef uint32_t
...
inline int64_t LoomDeviceMultiprocessorCount(int device_id) {
...
#define LOOM_HAS_OVERSIZED_SMEM 1
#else
#define LOOM_HAS_OVERSIZED_SMEM 0
#endif
...
inline bool LoomConfigureDynamicSmem(tvm::ffi::CubinKernel& kernel, int device_id,
  ...
  {
    using LoomCuKernelSetAttributeFn = int (*)(int attrib, int val, void* kernel, int dev);
    static const auto loom_cu_kernel_set_attribute =
        reinterpret_cast<LoomCuKernelSetAttributeFn>(dlsym(RTLD_DEFAULT, "cuKernelSetAttribute"));
    TVM_FFI_CHECK(loom_cu_kernel_set_attribute != nullptr, RuntimeError)
        << "cuKernelSetAttribute not resolvable while enabling the oversized shared-memory mode";
    int drv_err = loom_cu_kernel_set_attribute(
...
void Run_bf16_h32_topk128x_early_v47(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_partial_O, TensorView arg_partial_lse, TensorView arg_O, TensorView arg_partition_arrivals, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_sparse_topk, int64_t arg_num_splits, int64_t arg_num_head_tiles, int64_t arg_has_sinks, int64_t arg_completion_base, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  void* p_partition_arrivals = arg_partition_arrivals.data_ptr();
  ...
  status = cudaFuncSetAttribute(kernel_cake_dsv4_bf16_h32_topk128x_early_v47,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      143104);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_dsv4_bf16_h32_topk128x_early_v47) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(
      reinterpret_cast<const void*>(kernel_cake_dsv4_bf16_h32_topk128x_early_v47), grid, block, kargs,
      143104u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_dsv4_bf16_h32_topk128x_early_v47 launch failed: " << cudaGetErrorString(status);
}

}  // namespace flashinfer::cake_dsv4

TVM_FFI_DLL_EXPORT_TYPED_FUNC(
    run_bf16_h32_topk128x_early_v47, flashinfer::cake_dsv4::Run_bf16_h32_topk128x_early_v47);
