...
#define SMEM_SMEM_ACC_SCALE_STAGE_BYTES 256
#define SMEM_SMEM_ACC_SCALE_STRIDE 256
#define SMEM_SMEM_SUM_OFF 197888
...
kernel_cake_dsv4_bf16_h64_guard_single_tile_r23(CakeTensorMap const* tmap_q, CakeTensorMap const* tmap_swa_kv, CakeTensorMap const* tmap_compressed_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int sparse_topk, int batch_size, int max_q_len, int ragged_query, int has_sinks)
{
    ...
    #define pv_done_addr (mbar_base + 128)
    #define staged_o2_ready_addr (mbar_base + 136)
    #define staged_o2_empty_addr (mbar_base + 144)
    #define staged_o3_ready_addr (mbar_base + 152)
    #define staged_o3_empty_addr (mbar_base + 160)
    ...
    float* smem_sum = reinterpret_cast<float*>(smem_raw + 197888);
    const int smem_sum_addr = smem + 197888;
    int* smem_indices = reinterpret_cast<int*>(smem_raw + 198144);
    const int smem_indices_addr = smem + 198144;

    // Mbarrier init (13 pipeline groups, 0 ordered-sequence groups, 22 barriers)
    ...
    if (warp == 0) {
        ...
        if (leader) {
            ...
            // stats: 1 barriers, init_count=128
        ...
        { // softmax_main
            ...
            for (int tile = 0; tile < 1; tile++) {
                if (tile_count > tile) {
                    ...
                    if (col_half == 0) {
                        smem_acc_scale[tile * 64 + my_row] = acc_scale;
                    }
                    mbarrier_arrive(stats_addr + (tile) * 8);
            ...
            if (sparse_topk == 128) {
                ...
                float _rcp_0 = approx_rcp(smem_sum[my_row]);
                float inv_sum = _rcp_0;
                int output_base = (query_idx * num_heads + my_row) * 512;
                #pragma unroll
        ...
        { // correction_o2_main
            ...
            for (int tile_1 = 0; tile_1 < 1; tile_1++) {
                if (tile_count_1 > tile_1) {
                    ...
                    mbarrier_wait(stats_addr + (tile_1) * 8, 0);
                    float acc_scale_1 = smem_acc_scale[tile_1 * 64 + my_row_1];
                    if (tile_1 > 0) {
                    ...
                    for (int col = 0; col < 64; col++) {
                        staged_o2[col] = ((tile_1 == 0) ? _tmem_load_3[col] : staged_o2[col] + _tmem_load_3[col]);
                    }
                    asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
                    asm volatile("tcgen05.fence::before_thread_sync;");
                    if (elect_sync()) {
            ...
            asm volatile("barrier.sync 8, 384;" ::: "memory");
            asm volatile("tcgen05.fence::after_thread_sync;");
            float _rcp_1 = approx_rcp(smem_sum[my_row_1]);
            float inv_sum_1 = _rcp_1;
            int output_base_1 = (query_idx_1 * num_heads + my_row_1) * 512;
            if (sparse_topk > 128) {
                #pragma unroll
                for (int v_stage_2 = 0; v_stage_2 < 2; v_stage_2++) {
                    int output_col_2 = 256 + v_stage_2 * 128;
                    int output_addr_2 = taddr + (unsigned int)output_col_2 + (unsigned int)(tmem_row_origin_1 << 16);
                    ...
                    asm volatile(
                        ...
                        : "r"(output_addr_2 + 32));
                    int col_base_1 = v_stage_2 * 128 + col_half_1 * 64;
                    if (my_row_1 < num_heads) {
                        #pragma unroll
                        for (int offset_1 = 0; offset_1 < 64; offset_1 += 8) {
                            {
                                const float2 _prescale2_2 = {inv_sum_1 * output_scale_1, inv_sum_1 * output_scale_1};
                                #if __CUDA_ARCH__ >= 1000
                                #pragma unroll
                                for (int _ps = 0; _ps < 4; _ps++)
                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_4[offset_1])[_ps], _prescale2_2);
                                #else
                                #pragma unroll
                                for (int _ps = 0; _ps < 8; _ps++)
                                    _tmem_load_4[offset_1 + _ps] *= inv_sum_1 * output_scale_1;
                                #endif
                                __nv_bfloat162 _pk[4];
                                _pk[0] = __floats2bfloat162_rn(_tmem_load_4[offset_1 + 0], _tmem_load_4[offset_1 + 1]);
                                _pk[1] = __floats2bfloat162_rn(_tmem_load_4[offset_1 + 2], _tmem_load_4[offset_1 + 3]);
                                _pk[2] = __floats2bfloat162_rn(_tmem_load_4[offset_1 + 4], _tmem_load_4[offset_1 + 5]);
                                _pk[3] = __floats2bfloat162_rn(_tmem_load_4[offset_1 + 6], _tmem_load_4[offset_1 + 7]);
                                *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(O + (output_base_1 + col_base_1 + offset_1)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                            }
                        }
                    }
                }
            ...
            if (my_row_1 < num_heads) {
                ...
                for (int offset_2 = 0; offset_2 < 64; offset_2 += 8) {
                    {
                        const float2 _prescale2_3 = {inv_sum_1 * output_scale_1, inv_sum_1 * output_scale_1};
                        #if __CUDA_ARCH__ >= 1000
                        #pragma unroll
                        for (int _ps = 0; _ps < 4; _ps++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>(&staged_o2[offset_2])[_ps], _prescale2_3);
                        #else
                        #pragma unroll
                        for (int _ps = 0; _ps < 8; _ps++)
                            staged_o2[offset_2 + _ps] *= inv_sum_1 * output_scale_1;
                        #endif
                        __nv_bfloat162 _pk[4];
                        _pk[0] = __floats2bfloat162_rn(staged_o2[offset_2 + 0], staged_o2[offset_2 + 1]);
                        _pk[1] = __floats2bfloat162_rn(staged_o2[offset_2 + 2], staged_o2[offset_2 + 3]);
                        _pk[2] = __floats2bfloat162_rn(staged_o2[offset_2 + 4], staged_o2[offset_2 + 5]);
                        _pk[3] = __floats2bfloat162_rn(staged_o2[offset_2 + 6], staged_o2[offset_2 + 7]);
                        *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(O + (output_base_1 + staged_col_base + offset_2)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                    }
                }
            }
            asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // correction_o3_main
            ...
            const int my_row_2 = (unsigned int)logical_row_origin_2 + lane % 16;
            ...
            for (int tile_2 = 0; tile_2 < 1; tile_2++) {
                if (tile_count_2 > tile_2) {
                    ...
                    mbarrier_wait(stats_addr + (tile_2) * 8, 0);
                    float acc_scale_2 = smem_acc_scale[tile_2 * 64 + my_row_2];
                    if (tile_2 > 0) {
                    ...
                    for (int col_1 = 0; col_1 < 64; col_1++) {
                        staged_o3[col_1] = ((tile_2 == 0) ? _tmem_load_5[col_1] : staged_o3[col_1] + _tmem_load_5[col_1]);
                    }
                    asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
                    asm volatile("tcgen05.fence::before_thread_sync;");
                    if (elect_sync()) {
            ...
            asm volatile("barrier.sync 8, 384;" ::: "memory");
            asm volatile("tcgen05.fence::after_thread_sync;");
            float _rcp_2 = approx_rcp(smem_sum[my_row_2]);
            float inv_sum_2 = _rcp_2;
            int output_base_2 = (query_idx_2 * num_heads + my_row_2) * 512;
            int staged_col_base_1 = 384 + col_half_2 * 64;
            if (my_row_2 < num_heads) {
                #pragma unroll
                for (int offset_3 = 0; offset_3 < 64; offset_3 += 8) {
                    {
                        const float2 _prescale2_1 = {inv_sum_2 * output_scale_2, inv_sum_2 * output_scale_2};
                        #if __CUDA_ARCH__ >= 1000
                        #pragma unroll
                        for (int _ps = 0; _ps < 4; _ps++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>(&staged_o3[offset_3])[_ps], _prescale2_1);
                        #else
                        #pragma unroll
                        for (int _ps = 0; _ps < 8; _ps++)
                            staged_o3[offset_3 + _ps] *= inv_sum_2 * output_scale_2;
                        #endif
                        __nv_bfloat162 _pk[4];
                        _pk[0] = __floats2bfloat162_rn(staged_o3[offset_3 + 0], staged_o3[offset_3 + 1]);
                        _pk[1] = __floats2bfloat162_rn(staged_o3[offset_3 + 2], staged_o3[offset_3 + 3]);
                        _pk[2] = __floats2bfloat162_rn(staged_o3[offset_3 + 4], staged_o3[offset_3 + 5]);
                        _pk[3] = __floats2bfloat162_rn(staged_o3[offset_3 + 6], staged_o3[offset_3 + 7]);
                        *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(O + (output_base_2 + staged_col_base_1 + offset_3)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                    }
                }
            }
            asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
    ...
    if (warp == 12) {
        { // mma_warp_main
            ...
            asm volatile("tcgen05.fence::after_thread_sync;");
            #pragma unroll
            for (int tile_3 = 0; tile_3 < 1; tile_3++) {
                if (tile_count_3 > tile_3) {
                    ...
                    int _mma_b_lo_2 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (2) * 2048);
                    asm volatile(
                    "{\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"((tmem_tmem + (staged_col_2))), "r"(_mma_b_lo_2), "r"(tmem_tmem + (score_col_1 + 64)), "r"(0));
                    elect_commit(staged_o2_ready_addr);
                    elect_commit(kv_empty_addr + 16);
                    int _mma_b_lo_3 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (0) * 2048);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"((tmem_tmem + (256))), "r"(_mma_b_lo_3), "r"(tmem_tmem + (score_col_1 + 64)), "r"(((tile_3 == 0) ? 0 : 1)));
                    elect_commit(kv_empty_addr);
                    mbarrier_wait(staged_o2_empty_addr, tile_3 & 1);
                    int _mma_b_lo_4 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (3) * 2048);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"((tmem_tmem + (staged_col_2))), "r"(_mma_b_lo_4), "r"(tmem_tmem + (score_col_1 + 64)), "r"(0));
                    elect_commit(staged_o3_ready_addr);
                    elect_commit(kv_empty_addr + 24);
                    int _mma_b_lo_5 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (1) * 2048);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"((tmem_tmem + (384))), "r"(_mma_b_lo_5), "r"(tmem_tmem + (score_col_1 + 64)), "r"(((tile_3 == 0) ? 0 : 1)));
    ...
    if (warp >= 13 && warp <= 15) {
        { // load_warp_main
            ...
            #pragma unroll
            for (int sparse_tile = 0; sparse_tile < 1; sparse_tile++) {
                if (tile_count_4 > sparse_tile) {
                    if (load_warp_rank < 2) {
                        if (lane < 16) {
                            int index_group = (unsigned int)(load_warp_rank * 16) + lane;
                            int index_offset = sparse_tile * 128 + index_group * 4;
                            if (index_offset + 3 < sparse_topk) {
                                asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                                    :: "r"(smem_indices_addr + (unsigned int)(index_offset * 4)), "l"(sparse_indices + (sparse_base + index_offset)));
                            } else {
                                int rows[4];
                                #pragma unroll
                                for (int row_i = 0; row_i < 4; row_i++) {
                                    rows[row_i] = -1;
                                    if (index_offset + row_i < sparse_topk) {
                                        rows[row_i] = sparse_indices[sparse_base + index_offset + row_i];
                                    }
                                }
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(smem_indices_addr + (unsigned int)(index_offset * 4)), "r"(rows[0]), "r"(rows[1]), "r"(rows[2]), "r"(rows[3]) : "memory");
                            }
                        }
                    }
                }
            ...
            #pragma unroll
            for (int tile_4 = 0; tile_4 < 1; tile_4++) {
