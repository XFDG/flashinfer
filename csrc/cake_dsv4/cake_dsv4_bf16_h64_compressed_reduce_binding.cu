...
#include "cake_dsv4_bf16_h64_compressed_reduce.cu"
#undef CakeTensorMap
#undef uint8_t
#undef uint16_t
#undef uint32_t
...
void Run_bf16_h64_compressed_reduce(TensorView arg_partial_O, TensorView arg_partial_lse, TensorView arg_O, int64_t arg_num_heads, int64_t arg_num_splits, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  TVM_FFI_CHECK(arg_num_splits >= 1 && arg_num_splits <= 5, ValueError)
      << "num_splits must be in [1, 5] for bf16_h64_compressed_reduce";
  ...
  status = cudaLaunchKernel(
      reinterpret_cast<const void*>(kernel_cake_dsv4_bf16_h64_compressed_reduce), grid, block, kargs,
      0u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_dsv4_bf16_h64_compressed_reduce launch failed: " << cudaGetErrorString(status);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(
    run_bf16_h64_compressed_reduce, flashinfer::cake_dsv4::Run_bf16_h64_compressed_reduce);
