...
kernel_cake_dsv4_bf16_h32_topk128x_v40(CakeTensorMap const* tmap_q, CakeTensorMap const* tmap_swa_kv, CakeTensorMap const* tmap_compressed_kv, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, __nv_bfloat16* __restrict__ O, unsigned int* __restrict__ partition_arrivals, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int sparse_topk, int num_splits, int num_head_tiles, int has_sinks, int completion_base)
{
    ...
    #define o_full_lo_addr (mbar_base + 88)
    #define o_full_hi_addr (mbar_base + 104)
    #define tmem_dealloc_addr (mbar_base + 120)
    #define partial_done_addr (mbar_base + 128)
    ...
    if (warp >= 13 && warp <= 15) {
        { // load_warp_main
            ...
            for (int v_stage = 0; v_stage < 4; v_stage++) {
                if (v_stage % 3 == load_rank) {
                    mbarrier_wait(kv_empty_addr + (v_stage) * 8, 0);
                    if (elect_sync()) {
                        mbarrier_arrive_expect_tx(kv_full_addr + (v_stage) * 8, gather_tx_bytes);
                    }
                    if ((unsigned int)valid_groups > lane) {
                        int dst_v = smem_v_addr + (unsigned int)(v_stage * 32768);
                        if (split_idx_3 == 0) {
                            tma_gather4_gmem2smem((unsigned int)dst_v + lane * 512, tmap_swa_kv, v_stage * 128, row0, row1, row2, row3, kv_full_addr + (v_stage) * 8);
                            tma_gather4_gmem2smem((unsigned int)(dst_v + 16384) + lane * 512, tmap_swa_kv, v_stage * 128 + 64, row0, row1, row2, row3, kv_full_addr + (v_stage) * 8);
                        } else {
                            tma_gather4_gmem2smem((unsigned int)dst_v + lane * 512, tmap_compressed_kv, v_stage * 128, row0, row1, row2, row3, kv_full_addr + (v_stage) * 8);
                            tma_gather4_gmem2smem((unsigned int)(dst_v + 16384) + lane * 512, tmap_compressed_kv, v_stage * 128 + 64, row0, row1, row2, row3, kv_full_addr + (v_stage) * 8);
                        }
                    }
                }
            }
