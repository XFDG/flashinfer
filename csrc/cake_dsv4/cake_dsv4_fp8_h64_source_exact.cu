...
kernel_cake_dsv4_fp8_h64_source_exact(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ cum_seq_lens_q, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int sparse_topk, int has_sinks, int total_work_items)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            for (unsigned int _work = 0; _work < total_work_items; _work++) {
                ...
                softmax_q = _clc_ctaid_x_3;
                softmax_head = _clc_ctaid_y_3;
                softmax_batch = _clc_ctaid_z_3;
            }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_main
            ...
            for (unsigned int _work_1 = 0; _work_1 < total_work_items; _work_1++) {
                ...
                correction_q = _clc_ctaid_x_4;
                correction_head = _clc_ctaid_y_4;
                correction_batch = _clc_ctaid_z_4;
            }
            asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
    ...
    if (warp == 8) {
        { // mma_main
            unsigned int kv_cons_stage = 0;
            unsigned int kv_cons_phase = 0;
            ...
            for (unsigned int _work_2 = 0; _work_2 < total_work_items; _work_2++) {
                ...
                if (work_valid_2 != 0) {
                    ...
                    if (num_tiles_2 > 0) {
                        ...
                        mbarrier_wait(kv_full_addr + (kv_cons_stage) * 8, kv_cons_phase);
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        int _mma_a_lo_0 = make_warp_uniform((((smem_q_addr) >> 4) & 0x3FFF) + (q_cons_stage) * 2048);
                        int _mma_b_lo_0 = make_warp_uniform((((smem_k_addr) >> 4) & 0x3FFF) + (kv_cons_stage) * 2048);
                        {
                        ...
                        elect_commit(kv_empty_addr + (kv_cons_stage) * 8);
                        kv_cons_stage += 1;
                        if (kv_cons_stage == 4) { kv_cons_stage = 0; kv_cons_phase ^= 1; }
                        mbarrier_wait(kv_full_addr + (kv_cons_stage) * 8, kv_cons_phase);
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        int _mma_a_lo_1 = make_warp_uniform((((smem_q_addr + 16384) >> 4) & 0x3FFF) + (q_cons_stage) * 2048);
                        int _mma_b_lo_1 = make_warp_uniform((((smem_k_addr) >> 4) & 0x3FFF) + (kv_cons_stage) * 2048);
                        {
                        ...
                        for (int _steady = 0; _steady < num_tiles_2 - 1; _steady++) {
                            ...
                            int _mma_b_lo_2 = make_warp_uniform((((smem_k_addr) >> 4) & 0x3FFF) + (kv_cons_stage) * 2048);
                            {
                            ...
                            int _mma_b_lo_3 = make_warp_uniform((((smem_k_addr) >> 4) & 0x3FFF) + (kv_cons_stage) * 2048);
                            {
                            ...
                            for (int v_half = 0; v_half < 2; v_half++) {
                                ...
                                int _mma_b_lo_4 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (kv_cons_stage) * 2048);
                                asm volatile(
                    "{\n\t"
                    ...
                    :: "r"((tmem_tmem + (256 + row_bank_2))), "r"(_mma_b_lo_4), "r"(tmem_tmem + p_col_1), "r"(((first_pv) ? 0 : 1)));
                        ...
                        for (int v_half_1 = 0; v_half_1 < 2; v_half_1++) {
                            ...
                            int _mma_b_lo_5 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (kv_cons_stage) * 2048);
                            asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                ...
                mma_q = _clc_ctaid_x_2;
                mma_head = _clc_ctaid_y_2;
                mma_batch = _clc_ctaid_z_2;
            }
        }
    }
    // ---- Role: page_loader ----
    if (warp == 9) {
        { // page_loader_main
            ...
            for (unsigned int _work_3 = 0; _work_3 < total_work_items; _work_3++) {
                ...
                page_q = _clc_ctaid_x_0;
                page_head = _clc_ctaid_y_0;
                page_batch = _clc_ctaid_z_0;
            }
        }
    }
    // ---- Role: scheduler ----
    ...
    if (warp >= 12 && warp <= 15) {
        ...
        { // load_main
            ...
            for (unsigned int _work_6 = 0; _work_6 < total_work_items; _work_6++) {
                ...
                load_q = _clc_ctaid_x_1;
                load_head = _clc_ctaid_y_1;
                load_batch = _clc_ctaid_z_1;
            }
        }
    }
