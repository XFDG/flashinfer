...
kernel_cake_dsv4_bf16_h128_topk4x_v51(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_k, const __grid_constant__ CUtensorMap tmap_compressed_k, const __grid_constant__ CUtensorMap tmap_swa_v, const __grid_constant__ CUtensorMap tmap_compressed_v, __nv_bfloat16* __restrict__ O, float* __restrict__ partial_lse, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int num_query_tokens, int sparse_topk, int has_sinks, int total_work_items)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_wg_main
            ...
            for (unsigned int work_idx = cluster_id; work_idx < total_work_items; work_idx += num_clusters) {
                ...
                int num_kv_tiles = ((sparse_topk + 128 - 1) / 128 + ((1) ? 5 : 1) - 1) / ((1) ? 5 : 1);
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_wg_main
            ...
            for (unsigned int work_idx_1 = cluster_id; work_idx_1 < total_work_items; work_idx_1 += num_clusters) {
                ...
                int num_kv_tiles_1 = ((sparse_topk + 128 - 1) / 128 + ((1) ? 5 : 1) - 1) / ((1) ? 5 : 1);
    ...
    if (warp == 8) {
        { // mma_warp_main
            ...
            for (unsigned int work_idx_2 = cluster_id; work_idx_2 < total_work_items; work_idx_2 += num_clusters) {
                int num_kv_tiles_2 = ((sparse_topk + 128 - 1) / 128 + ((1) ? 5 : 1) - 1) / ((1) ? 5 : 1);
                if (cta_rank == 0) {
    ...
    if (warp >= 9 && warp <= 12) {
        { // load_wg_main
            ...
            for (unsigned int work_idx_3 = cluster_id; work_idx_3 < total_work_items; work_idx_3 += num_clusters) {
                ...
                int num_kv_tiles_3 = ((sparse_topk + 128 - 1) / 128 + ((1) ? 5 : 1) - 1) / ((1) ? 5 : 1);
