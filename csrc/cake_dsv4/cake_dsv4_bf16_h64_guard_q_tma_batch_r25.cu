    ...
    if (warp >= 5 && warp <= 7) {
        { // load_warp_main
            ...
            if (group < 32) {
                ...
                int _vec_load_0[4];
                {
                    int4 _iv4 = *reinterpret_cast<const int4*>(sparse_indices + (sparse_base + index_base) + 0);
